...
async function create(data) {
  try {
    ...
    const role = await roleRepo.getRoleByType(
      Enums.USER_ROLES_ENUMS.RETAIL_USER
    );
    user.addRole(role);
    return user;
  } catch (error) {
...
// Service to add a role to a user
async function addRoletoUser(data) {
  try {
    ...
    const role = await roleRepo.getRoleByType(data.role);
    if (!role) {
      throw new AppError(
...
// Service to check if the user is an admin
async function isAdmin(id) {
  try {
    ...
    const adminrole = await roleRepo.getRoleByType(
